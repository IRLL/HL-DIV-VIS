...
def highlights_div(args, state_importance_df, budget, context_length, minimum_gap, distance_metric=distance.euclidean, percentile_threshold=3, subset_threshold = 1000):
...
#        print(state_importance_df)
    ...
    state_features = np.random.choice(state_features, size=subset_threshold, replace=True)
    distances = []
    for i in range(len(state_features-1)):
        #logger.info("iterating through state_features length")
...
#                print('summary_states:', summary_states)
        ...
        for state in summary_states:
            ...
            left_index = int(max(state - context_length,min_state))
            right_index = int(min(state + context_length,max_state) +1)
